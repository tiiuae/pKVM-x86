...
static u64 read_guest_phys(struct kvm_vcpu *vcpu, u64 phys, int stage)
{
	 ...
	 */
	if ((vcpu->kvm->pkvm.shadow_vm_handle == PKVM_HOST_HANDLE) ||
	    (stage == 1)) {
 ...
 */
int print_guest_maps_by_handle(int shadow_vm_handle)
{
...
static int __print_guest_maps(struct kvm_vcpu *vcpu, dtype_t table)
{
	...
	if (vcpu->kvm->pkvm.shadow_vm_handle != PKVM_HOST_HANDLE) {
		vm = get_shadow_vm(vcpu->kvm->pkvm.shadow_vm_handle);
		if (!vm) {
			pkvm_err("No such vm 0x%x\n",
				 vcpu->kvm->pkvm.shadow_vm_handle);
			return -ENOENT;
		}
	}
